...
export default function AtsScannerView({ 
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-8">
      ...
      {/* Top Header: Simple, spacious, and contextual */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Evaluating your resume match for <strong className="text-slate-800">{activeDrive.company}</strong> ({activeDrive.role})
          </p>
        </div>
      ...
      {/* Step 1: Resume Input Card (Spacious & Clean) */}
      <div className="p-8 rounded-3xl light-card border border-slate-200 space-y-6">
        <div>
          ...
          <h2 className="text-lg font-bold text-slate-900">Select or Upload Your Resume</h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Choose a benchmark profile for instant viva evaluation or upload your own file (.pdf, .docx, .txt).
          </p>
        </div>
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl text-left border transition-all cursor-pointer ${
                  isSelected
                    ? 'border-indigo-600 bg-indigo-50/60 ring-2 ring-indigo-500/10 shadow-xs'
                    : 'border-slate-200 bg-slate-50/60 hover:bg-white hover:border-slate-300'
                }`}
              >
                <div className="text-xs font-bold text-slate-900">{sample.name.split(' ')[0]}</div>
                <div className="text-[11px] text-slate-500 truncate mt-0.5">
                  {key === 'highMatch' ? 'Top-Tier CS Candidate' : key === 'averageMatch' ? 'Average Profile' : 'Missing Tech Keywords'}
                </div>
                <div className={`text-[10px] px-2 py-0.5 rounded-full mt-2 font-semibold inline-block ${
        ...
        <div className="relative border-2 border-dashed border-slate-200 hover:border-indigo-400 rounded-2xl p-6 text-center bg-slate-50/50 transition-colors">
          <input
            type="file"
          ...
          {showRawEditor && (
            <div className="mt-3 space-y-2 animate-in fade-in">
              <textarea
                ...
                rows={8}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-800 font-mono focus:ring-2 focus:ring-indigo-500 focus:bg-white outline-none leading-relaxed resize-none"
              />
            </div>
          )}
        </div>
      ...
      {/* Step 2: Instant ATS Diagnostics Card (Reveals cleanly below) */}
      {scanResult && (
        <div className="p-8 rounded-3xl light-card border border-slate-200 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div>
              ...
              <h2 className="text-lg font-bold text-slate-900">ATS Evaluation & Feedback</h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Target Cutoff: <strong className="text-slate-700">{activeDrive.minAtsScore}%</strong> for {activeDrive.company}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <span className="text-xs font-bold text-slate-500 block">Match Score</span>
              ...
              <div className={`p-2.5 rounded-2xl ${
                scanResult.passedCutoff ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
              }`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
              <div className="text-xs text-slate-500 font-semibold">Hard Skills Match</div>
              <div className="text-xl font-bold text-slate-900 mt-1">{scanResult.hardSkillsMatch}%</div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
              <div className="text-xs text-slate-500 font-semibold">Quantification (Impact)</div>
              <div className="text-xl font-bold text-slate-900 mt-1">{scanResult.quantificationScore}%</div>
              <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
              <div className="text-xs text-slate-500 font-semibold">Format & Contacts</div>
              <div className="text-xl font-bold text-slate-900 mt-1">{scanResult.sectionsFound.length} / 4 Sections</div>
              <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
          ...
          <div className="space-y-4 pt-2">
            <div>
              <span className="text-xs font-bold text-emerald-800 flex items-center gap-1.5 mb-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Matched Skills ({scanResult.foundHardSkills.length}):
              </span>
            ...
            {scanResult.missingHardSkills.length > 0 && (
              <div>
                <span className="text-xs font-bold text-amber-800 flex items-center gap-1.5 mb-2">
                  <AlertTriangle className="w-3.5 h-3.5 text-amber-600" /> Missing Job Keywords ({scanResult.missingHardSkills.length}):
                </span>
                <div className="flex flex-wrap gap-1.5">
          ...
          {/* Guidance or Red Flags */}
          {scanResult.redFlags.length > 0 && (
            <div className="p-4 rounded-2xl bg-red-50 border border-red-200">
              <span className="text-xs font-bold text-red-800 block mb-1">ATS Optimization Checklist:</span>
          ...
          <div className="pt-4">
            <button
              ...
              className={`w-full py-3.5 px-6 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 transition-all cursor-pointer ${
